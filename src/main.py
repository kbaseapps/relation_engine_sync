...
def update_provenance(params):
    ...
    """
    uid = params['uid']
    print('Running for %s' % uid)
    workspaces = workspace_client.admin_req('listWorkspaces', {'owners': [uid]})
    print('Found %s workspaces' % len(workspaces))
    ...
    print('%s total workspaces' % len(workspaces))
    ...
    if not len(db_objects):
        ...
        return {}
    print("%s total workspace objects" % len(db_objects))
    ...
    ws_objects = workspace_client.admin_req('getObjects', {
    ...
    print('Importing %s objects' % len(db_objects))
    re_client.save(_obj_vert_name, db_objects)
    print('Importing %s copies' % len(db_copies))
    re_client.save(_copy_edge_name, db_copies)
    print('Importing %s %s' % (len(db_links), _link_edge_name))
